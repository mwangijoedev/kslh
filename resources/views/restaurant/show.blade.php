<x-layout1>
    <body class="bg-gray-950">
        <div class="bg-gray-950 pb-20 select-none">
            <div class="pt-6">
                <header>
                    <h2 class="text-gray-300 my-2 text-lg sm:text-4xl text-center font-audiowide">{{$restaurant->name}}</h2>
                </header>
                <div class="mx-auto max-w-2xl px-4 pt-10 pb-16 sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:grid-rows-[auto_auto_1fr] lg:gap-x-8 lg:px-8 lg:pt-16 lg:pb-24">
                    <div class="lg:col-span-2 lg:border-r lg:border-gray-200 lg:pr-8">
                        <h1 class="text-2xl font-bold animate-pulse tracking-tight text-gray-900 sm:text-3xl">{{ $restaurant->name }}</h1>
                    </div>
                    <div class="mt-4 lg:row-span-3 lg:mt-0">
                        <div class=" max-w-2xl sm:px-2 ">
                            ...
                            <div class="mt-5 text-amber-800 text-center">
                              <a href="/restaurant/show/{{ $next->id }}" class="text-sm cursor-pointer font-bold ">View&nbsp;{{ $next->name }}
                    ...
                    <div class="py-10 lg:col-span-2 lg:col-start-1 lg:border-r lg:border-gray-200 lg:pt-6 lg:pr-8 lg:pb-16">
                        <div>
                            ...
                            <div class="space-y-6">
                                <p class="text-base text-gray-300">✨🚀{{$restaurant->description}}! 🌴🐘🌊 
                                </p>
                            </div>
                        ...
                        <form method="POST" action="/book/{{ $restaurant->id }}" class="hidden sm:block mt-2 mb-10 sm:flex sm:justify-end">
                            @csrf
            ...
            <div class="mx-auto max-w-2xl sm:px-6 lg:grid lg:max-w-7xl lg:grid-cols-3 lg:gap-x-8 lg:px-8">
                <div class="col-span-1 mt-4 lg:row-span-3 lg:mt-0">
                    <a href="/hotel/{{ $restaurant->hotel_tag }}" class="text-3xl tracking-tight text-gray-700 uppercase">{{ $restaurant->hotel_tag }}</a>
                    <div class="mt-6">
                    ...
                    <form method="POST" action="/book/{{ $restaurant->id }}" class="mt-10">
                        @csrf
                        <div class="mt-10">
                            <div class="flex items-center justify-between">
                                <h3 class="text-sm font-medium text-gray-900">{{ $restaurant->service_tag }}</h3>
                                <a href="/hotel/show/{{ $restaurant->hotel_tag }}" class="text-sm font-medium text-indigo-600 hover:text-indigo-500">
                            ...
                            <fieldset aria-label="Package Inclusions" class="mt-4">
                                <div class="grid grid-cols-4 gap-4 sm:grid-cols-8 lg:grid-cols-4">
                                        <label class="animate-pulse hover relative flex cursor-pointer items-center justify-center rounded-md border bg-white px-4 py-3 text-sm font-medium text-gray-900 shadow-xs hover:bg-gray-50 focus:outline-hidden sm:flex-1 sm:py-6">
                                            <span class="text-gray-600 text-xs text-center">✔ {{ $restaurant->capacity }}</span>

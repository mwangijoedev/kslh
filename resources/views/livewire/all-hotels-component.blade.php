<div class="bg-gray-950 min-h-screen py-10 px-4 sm:px-6 lg:px-8">
    <h2 class="text-center text-3xl font-audiowide text-gray-200 mb-10">Explore Our Hotels</h2>

    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        @foreach($hotels as $hotel)
            <div wire:key="{{ $hotel->id }}" class="bg-gray-900 rounded-xl shadow-md hover:shadow-lg transition duration-300 overflow-hidden">
                ...
                <div class="p-6">
                    <h3 class="text-xl font-semibold text-amber-400 mb-2">{{ $hotel->name }}</h3>
                    <p class="text-gray-300 text-sm mb-4">
                        {{ \Illuminate\Support\Str::words(strip_tags($hotel->description), 20, '...') }}
                    </p>
                    <a href="/hotel/{{ $hotel->tag }}" wire:navigate

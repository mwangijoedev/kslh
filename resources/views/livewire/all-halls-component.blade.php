    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
        @foreach($halls as $hall)
            <div class="bg-gray-900 rounded-xl shadow-md hover:shadow-lg transition duration-300 overflow-hidden">
                ...
                <div class="p-6">
                    <h3 class="text-xl font-semibold text-amber-400 mb-2">{{ $hall->name }}</h3>
                    <p class="text-gray-300 text-sm mb-4">
                        {{ \Illuminate\Support\Str::words(strip_tags($hall->description), 20, '...') }}
                    </p>
                    <a href="/hall/{{ $hall->id }}" wire:navigate
